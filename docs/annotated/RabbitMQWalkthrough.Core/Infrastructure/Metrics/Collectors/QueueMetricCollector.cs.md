# RabbitMQWalkthrough.Core/Infrastructure/Metrics/Collectors/QueueMetricCollector.cs

## Propósito
Coleta métricas da fila via RabbitMQ Management API e as traduz para o modelo interno.
...
### Anotações técnicas
- **Integração com Management API**: `RestClient` consulta `/queues/Walkthrough/test_queue`.
- **Tratamento de falhas**: `try/catch` registra erro e evita quebrar o ciclo de coleta.
- **DTOs extensos**: classes internas mapeiam o JSON retornado pela API do RabbitMQ.
- **Boas práticas de observabilidade**: expõe `QueueSize`, `PublishRate` e `ConsumeRate` para dashboards.
