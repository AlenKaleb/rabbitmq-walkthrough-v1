# RabbitMQWalkthrough.Core/Infrastructure/Queue/Consumer.cs

## Propósito
Consumidor de mensagens com controle de throughput, confirmação manual (ack/nack) e persistência transacional.
...
public class Consumer
{
    ...
    public Consumer(IChannel channel, IConnection connection, ILogger<Consumer> logger,
        NpgsqlConnection sqlConnection, MessageDataService messageDataService)
    {
...
### Anotações técnicas
- **`AsyncEventingBasicConsumer`**: integra com modelo assíncrono do RabbitMQ.Client.
- **Controle de vazão**: `prefetch` ajustado com base na taxa desejada para reduzir latência.
- **Ack/Nack/Reject explícitos**: garante semântica de processamento exatamente uma vez (do ponto de vista do consumidor).
- **Transação com Postgres**: atualização de métricas de mensagem e confirmação do broker são coordenadas.
- **Resiliência a shutdown**: rejeita mensagens quando `isRunning` fica falso para não processar durante shutdown.
