# infra/postgres/01.init.sql

## Propósito
Cria o esquema `app` e as tabelas de mensagens/métricas no Postgres.
...
### Anotações técnicas
- **Schema isolado**: separa objetos do aplicativo (`app`).
- **Sequences**: geram IDs incrementais para mensagens e métricas.
- **Campos de métricas**: alinhados ao modelo `Metric`.
- **`TimeSpent`**: armazena duração do processamento para análise de latência.
